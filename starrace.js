...
if (window.innerWidth >= 600 && window.innerHeight >= 500) {
    ...
    countdownTimeout = null, countdownInterval = null, rockInterval = null, rocks = [], stars = [],
    moveAnimation = null, backgroundShiftX = 0, backgroundShiftY = 0,
    ...
    backgroundImage.width = 5000
    backgroundImage.height = 2812
    var backgroundX = -backgroundImage.width * 2,
    backgroundY = -backgroundImage.height * 2,
    backgroundWidth = backgroundImage.width * 4,
    backgroundHeight = backgroundImage.height * 4,
    ...
    function updateStarbux(amount) {
            ...
            chatInput.onkeyup = function(e) {
                ...
                countdownInterval = setInterval(() => {
                    ...
                    if (countdown.textContent == 0) {
                        ...
                        move()
                        if ((topVehicle.src == currentVehicle.src ? topVehicle : bottomVehicle) == bottomVehicle) {
                            ...
                            for (var i = 0; i < ((backgroundHeight * backgroundWidth) / 2) * .000001; i++) {
                            ...
                            } for (var i = 0; i < ((backgroundHeight * backgroundWidth) / 2) * .0000005; i++) {
                                var rockX = Math.floor(Math.random() * ((backgroundWidth + 300) - (backgroundX - 300)) + (backgroundX - 300))
                                var rockY = Math.floor(Math.random() * ((backgroundHeight + 300) - (backgroundY - 300)) + (backgroundY - 300))
                                socket.emit(`rock`, {image: [`rock1.png`, `rock2.png`, `rock3.png`][Math.floor(Math.random() * 3)], x: rockX, y: rockY, xSpeed: Math.floor(Math.random() * (6 - 3) + 3), ySpeed: Math.floor(Math.random() * (6 - 3) + 3), width: Math.floor(Math.random() * (100 - 25) + 25), height: Math.floor(Math.random() * (100 - 25) + 25)})
                            }
                        }
                    }
                }, 1000);
        ...
        if (!topVehicle.hidden && document.activeElement != chatInput && countdown.textContent == 0) {
            var raceVehicle = (topVehicle.src == currentVehicle.src ? topVehicle : bottomVehicle)
        ...
        ctx.clearRect(-backgroundImage.width * 30, -backgroundImage.height * 30, backgroundImage.width * 60, backgroundImage.height * 60)
        ...
        ctx.drawImage(backgroundImage, backgroundX - backgroundWidth, backgroundY - backgroundHeight, backgroundWidth, backgroundHeight)
        ctx.drawImage(backgroundImage, backgroundX + backgroundWidth, backgroundY + backgroundHeight, backgroundWidth, backgroundHeight)
        ctx.drawImage(backgroundImage, backgroundX - backgroundWidth, backgroundY, backgroundWidth, backgroundHeight)
        ctx.drawImage(backgroundImage, backgroundX, backgroundY - backgroundHeight, backgroundWidth, backgroundHeight)
        ctx.drawImage(backgroundImage, backgroundX + backgroundWidth, backgroundY, backgroundWidth, backgroundHeight)
        ctx.drawImage(backgroundImage, backgroundX, backgroundY + backgroundHeight, backgroundWidth, backgroundHeight)
        ...
        if (hitVehicle) {
            ...
            if (hitVehicleRock) {
                let newXSpeed = hitVehicleRock.getAttribute(`data-moveXSpeed`) - 0.05 * ((rock.width + rock.height) / 2)
                let newYSpeed = hitVehicleRock.getAttribute(`data-moveYSpeed`) - 0.05 * ((rock.width + rock.height) / 2)
                hitVehicleRock.setAttribute(`data-moveXSpeed`, newXSpeed)
                hitVehicleRock.setAttribute(`data-moveYSpeed`, newYSpeed)
                if (rocks[i] == rock) rocks.splice(i, 1)
                    if (currentVehicle.src == hitVehicleRock.src) {
                        updateStarbux(parseFloat(localStorage.getItem(`Starbux`)) - 5)
                    } if (newXSpeed <= 0 && newYSpeed <= 0) {
                        hitVehicleRock.setAttribute(`data-moveXSpeed`, 0)
                        hitVehicleRock.setAttribute(`data-moveYSpeed`, 0)
                        if (currentVehicle.src == hitVehicleRock.src) {
            ...
            if (hitVehicleStar) {
                ...
                hitVehicleStar.setAttribute(`data-moveYSpeed`, parseFloat(hitVehicleStar.getAttribute(`data-moveYSpeed`)) + 0.5)
        ...
        (vehicle == bottomVehicle.src ? bottomVehicle : topVehicle).setAttribute(attribute, value)
        if (currentVehicle.src == vehicle) {
    ...
    function mobileMove(e) {
        ...
        if (e.touches.length > 0 && !topVehicle.hidden && document.activeElement != chatInput && countdown.textContent == 0) {
            e.preventDefault()
            ...
            else if (touchX > parseFloat(raceVehicle.getAttribute(`data-canvasX`)) + raceVehicle.width) socket.emit(`moveVehicle`, raceVehicle.src, `data-xSpeed`, raceVehicle.getAttribute(`data-moveXSpeed`))
            else socket.emit(`moveVehicle`, raceVehicle.src, `data-xSpeed`, 0)
            ...
            else if (touchY > parseFloat(raceVehicle.getAttribute(`data-canvasY`)) + raceVehicle.height) socket.emit(`moveVehicle`, raceVehicle.src, `data-ySpeed`, raceVehicle.getAttribute(`data-moveYSpeed`))
    ...
    alert(`Your screen is not big enough to play this game. The requirement is a width minimum of 600 pixels and a height minimum of 500 pixels.`)
}